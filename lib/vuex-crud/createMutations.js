...
var createMutations = function createMutations(_ref) {
  ...
  if (only.includes('BATCH_UPDATE')) {
    Object.assign(crudMutations, {
      batchUpdateStart: function batchUpdateStart(state) {
        state.isBatchUpdating = true;
      },
      batchUpdateSuccess: function batchUpdateSuccess(state, response) {
        var data = response.data;

        data.forEach(function (entity) {
          var id = entity[idAttribute].toString();

          _vue2.default.set(state.entities, id, entity);
      ...
      batchUpdateError: function batchUpdateError(state, err) {
